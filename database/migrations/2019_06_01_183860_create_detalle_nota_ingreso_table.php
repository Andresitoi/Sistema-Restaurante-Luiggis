...
class CreateDetalleNotaIngresoTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('detalle_nota_ingreso', function (Blueprint $table) {
            ...
            $table->foreign('id_notaingreso','fk_idnotaingreso_detallenotaingreso')
            ->references('id')->on('nota_ingreso')->onUpdate('cascade')->onDelete('cascade');
            
            //asociado a la tabla insumo(id)
            $table->foreign('id_insumo','fk_idinsumo_detallenotaingreso')
            ->references('id')->on('insumo')->onUpdate('cascade')->onDelete('cascade');
